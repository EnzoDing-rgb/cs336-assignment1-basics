...
```bash
cd /root/.dev/ml-sys/cs336/assignment1-basics && uv run python scripts/tokenizer_metrics.py
```
...
`scripts/tokenizer_metrics.py` 与 handout 一致：
...
from scripts.tokenizer_metrics import load_tok, sample_documents, compression_ratio_bytes_per_token
ROOT = Path('.')
...
cd /root/.dev/ml-sys/cs336/assignment1-basics && uv run python -c "
from pathlib import Path
from scripts.tokenizer_metrics import load_tok, sample_documents
...
### 测量方法（`scripts/tokenizer_metrics.py`）
...
cd /root/.dev/ml-sys/cs336/assignment1-basics && uv run python -c "
from pathlib import Path
from scripts.tokenizer_metrics import load_tok, benchmark_throughput
ROOT = Path('.')
...
data/*.txt
  ...
  → Tokenizer.encode       → (a)(b)(c) 用 scripts/tokenizer_metrics.py
...
| 用途 | 路径 |
|------|------|
| (a)(b)(c)(d) 汇总脚本 | `scripts/tokenizer_metrics.py` |
...
以下内容由 `scripts/tokenizer_metrics.py` 打印，原封归档（原独立文件 `tokenizer_experiments_last_run.txt` 已删除并入此处）。

```text
========================================================================
CS336 tokenizer_metrics.py results
========================================================================
{
  "a": {
